...
int osc_start(struct deck *deck, struct library *library)
{
    ...
    lo_server_thread_add_method(st, "/xwax/library", "i", library_handler, NULL);
...
int library_handler(const char *path, const char *types, lo_arg ** argv,
                int argc, void *data, void *user_data)
{
...
int osc_send_record(lo_address a, int r)
{
    ...
    lo_send(a, "/client/get_record", "isss", r, rx->title, rx->artist, rx->pathname);
}
